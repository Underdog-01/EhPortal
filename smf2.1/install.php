...
elseif (!defined('SMF'))
	die('<b>Error:</b> Cannot install - please verify that your directory settings are correct.');
...
add_integration_function('integrate_pre_log_stats', '$sourcedir/PortalHooks.php|sportal_initialize');
add_integration_function('integrate_actions', '$sourcedir/PortalHooks.php|sportal_actions');
add_integration_function('integrate_admin_areas', '$sourcedir/PortalHooks.php|sportal_admin_areas');
add_integration_function('integrate_admin_search', '$sourcedir/PortalHooks.php|sportal_admin_search');
add_integration_function('integrate_user_info', '$sourcedir/PortalHooks.php|sportal_user_info');
add_integration_function('integrate_mark_read_button', '$sourcedir/PortalHooks.php|sportal_mark_read_button');
add_integration_function('integrate_display_message_list', '$sourcedir/PortalHooks.php|sportal_display_message_list');
add_integration_function('integrate_helpadmin', '$sourcedir/PortalHooks.php|sportal_helpadmin');
add_integration_function('integrate_prepare_db_settings', '$sourcedir/PortalHooks.php|sportal_prepare_db_settings');
add_integration_function('integrate_buffer', '$sourcedir/PortalHooks.php|sportal_buffer');
add_integration_function('integrate_pre_parsebbc', '$sourcedir/PortalHooks.php|sportal_pre_parsebbc');
...
foreach ($tables as $table => $data)
{
	...
	<li>"' . $table . '" table exists, updating table structure.
		...
		foreach ($data['columns'] as $column)
		{
			if (!isset($column['deprecated_name']) || !$smcFunc['db_change_column']('{db_prefix}' . $table, $column['deprecated_name'], $column))
				$smcFunc['db_add_column']('{db_prefix}' . $table, $column, array(), 'ignore', 'fatal');
		}
		...
		foreach ($data['indexes'] as $index)
			$smcFunc['db_add_index']('{db_prefix}' . $table, $index, array(), 'ignore', 'fatal');
	...
	{
		$smcFunc['db_create_table']('{db_prefix}' . $table, $data['columns'], $data['indexes'], array(), 'ignore', 'fatal');
